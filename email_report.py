...
from database import get_all_users, get_db
...
# ─── Récupération alertes depuis IMOU ────────────────────────────────────────
def get_weekly_alerts_by_site():
    """Récupère les alertes de la semaine passée depuis la DB IMOU via l'API"""
    from blueprint.camera import get_access_token, get_imou_alerts, normalize_alarm
    ...
    import requests
    from blueprint.camera import post_to_imou
    try:
    ...
    # Grouper les caméras par site
    def detect_site(name=""):
        n = name.upper()
    ...
    for device in device_list:
        ...
        # Récupérer les alertes de cette caméra
        try:
            ...
            human  = sum(1 for a in alarms if a["typeLabel"] in ("human_infrared",))
            motion = sum(1 for a in alarms if a["typeLabel"] == "motion_detection")
            other  = len(alarms) - human - motion

            sites_data[site]["cameras"].append({
                "name":   device_name,
                "total":  len(alarms),
                "human":  human,
                "motion": motion,
...
def generate_report_html(site_name, site_data, period_start, period_end):
    cameras_rows = ""
    for cam in sorted(site_data["cameras"], key=lambda x: x["total"], reverse=True):
        cameras_rows += f"""
        <tr>
            ...
            <td style="padding:8px 12px;border-bottom:1px solid #2d2d2d;text-align:center;color:#94a3b8">{cam['other']}</td>
  ...
  <div style="max-width:600px;margin:0 auto;padding:24px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:16px">
      ...
      <div style="background:#1e293b;border-radius:12px;padding:16px;text-align:center;border:1px solid #0e7490">
        <p style="margin:0;color:#64748b;font-size:11px;text-transform:uppercase">Détection humaine</p>
      ...
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr style="background:#0f172a">
            ...
            <th style="padding:10px 12px;text-align:center;color:#94a3b8;font-size:11px;text-transform:uppercase">Autre</th>
...
def send_weekly_reports():
    ...
    # Récupérer les données d'alertes
    sites_data = get_weekly_alerts_by_site()
    if not sites_data:
        print("⚠️ Aucune donnée d'alerte disponible")
        return

    # Récupérer les utilisateurs
    users = get_all_users()

    # Envoyer à chaque admin de site
    for user in users:
        ...
        if user["role"] == "admin_site" and user.get("site"):
            ...
            # Rapport consolidé pour le superadmin
  ...
  <div style="max-width:600px;margin:0 auto;padding:24px">
    ...
    <div style="display:flex;gap:12px;margin-bottom:16px">
      <div style="flex:1;background:#1e293b;border-radius:12px;padding:16px;text-align:center;border:1px solid #334155">
        <p style="margin:0;color:#64748b;font-size:11px;text-transform:uppercase">Total</p>
        <p style="margin:4px 0 0;color:#f8fafc;font-size:28px;font-weight:bold">{total_all}</p>
      </div>
      <div style="flex:1;background:#1e293b;border-radius:12px;padding:16px;text-align:center;border:1px solid #0e7490">
        ...
        <p style="margin:4px 0 0;color:#22d3ee;font-size:28px;font-weight:bold">{human_all}</p>
      </div>
      <div style="flex:1;background:#1e293b;border-radius:12px;padding:16px;text-align:center;border:1px solid #92400e">
        <p style="margin:0;color:#64748b;font-size:11px;text-transform:uppercase">Mouvement</p>
        <p style="margin:4px 0 0;color:#fbbf24;font-size:28px;font-weight:bold">{motion_all}</p>
      </div>
    </div>
    <div style="background:#1e293b;border-radius:12px;overflow:hidden;border:1px solid #334155;margin-bottom:16px">
      <div style="padding:16px;border-bottom:1px solid #334155">
